  ...
  useEffect(() => {
    async function fetchSuggestedQuizzes() {
      try {
        ...
        // Finde alle Quizze, die noch nicht begonnen wurden
              ...
              <div className="mt-2 px-4 py-3 bg-white dark:bg-gray-900 border border-indigo-100 dark:border-indigo-800 rounded-lg space-y-3">
                {suggestedQuizzes.length === 0 ? (
                  ...
                  suggestedQuizzes.map((quiz) => (
                    <div key={quiz.id} className="flex items-center justify-between py-2 border-b border-gray-100 dark:border-gray-800 last:border-b-0">
                      <div className="flex flex-col">
                        <span className="font-semibold text-gray-900 dark:text-white">{quiz.shortTitle || quiz.title}</span>
                      </div>
                      <button
                        className="ml-4 px-3 py-1 rounded-lg bg-indigo-600 text-white dark:bg-indigo-500 dark:text-white font-medium text-sm hover:bg-indigo-700 dark:hover:bg-indigo-600 transition-colors flex items-center gap-2"
