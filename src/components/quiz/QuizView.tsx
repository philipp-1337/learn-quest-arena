  ...
  useEffect(() => {
    const loadChallenges = async () => {
      try {
        ...
        const formattedChallenges: QuizChallenge[] = loadedChallenges.map(
          (challenge: {
            id: string;
            title?: string;
    ...
    const fetchFeaturedQuizzes = async () => {
      ...
      try {
        ...
        const visibleQuizzes = allQuizzes
          .filter(
            (q) =>
              !q.hidden && Array.isArray(q.questions) && q.questions.length > 3,
          )
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-stretch">
              {featuredLoading
                ? Array.from({ length: 3 }).map((_, i) => (
                    <div key={i} className="relative flex flex-col h-full animate-pulse">
        ...
        {!selectedSubject && challenges.length > 0 && isAuthenticated && (
          <div className="bg-gradient-to-br from-yellow-50 to-amber-50 dark:from-yellow-900/20 dark:to-amber-900/20 rounded-2xl shadow-lg p-6 mb-5">
            ...
            <p className="text-gray-700 dark:text-gray-300 mb-2">
              Stelle dich der ultimativen Herausforderung! Beantworte Fragen auf
              15 verschiedenen Schwierigkeitsstufen und gewinne bis zu 1 Million
              Euro!
            </p>
            <p className="text-sm text-purple-700 dark:text-purple-300 font-medium mb-4">
              <BadgeInfoIcon className="w-4 h-4 inline-block mr-1 mb-1" />{" "}
              Dieses Feature befindet sich in der Beta-Phase und ist nur für
              Lehrkräfte und Administration sichtbar.
            </p>
